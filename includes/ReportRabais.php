...
class ReportRabais extends Report
{
    ...
    { 
        ...
        $this->master = ["reimbursed"=>[], "subsid"=>[], "rabais-subsid"=>[]];
        ...
        $this->columnsCsv = ["reimbursed" => array_merge($this::D1, ["discount-bonus", "subsid-bonus"]), "subsid" => array_merge($this::D1, $this->dsub)];
    ...
    function generate($suffix)
    {
        ...
        if($this->factel == 7 || $this->factel == 8) {
            ...
            $lines = Csv::extract($this->dirRun."/Bilans_Stats/".$this->bilansStats[$this->factel]['Bilan-s']['prefix'].$suffix.".csv");
            for($i=1;$i<count($lines);$i++) {
                $tab = explode(";", $lines[$i]);
        ...
        $rabaisColumns = [$this->paramtext->getParam("client-code"), $this->paramtext->getParam("client-class"), $this->paramtext->getParam("item-codeD"), 
                            $this->paramtext->getParam("deduct-CHF"), $this->paramtext->getParam("subsid-deduct"), $this->paramtext->getParam("discount-bonus"), $this->paramtext->getParam("subsid-bonus")];
        Csv::write($this->dirRun."/REPORT/".$this->report[$this->factel][$this->reportKey]['prefix'].".csv", array_merge([$rabaisColumns], $rabaisArray));
        return $rabaisArray;
    }
    ...
    function masterise($rabaisArray)
    {
        foreach($rabaisArray as $line) {
            ...
            $dimensions = ["reimbursed"=>[$this::D1], "subsid"=>[$this::D1], "rabais-subsid"=>[$this::D1, $this::D2, $this::D3]];
            $sums = ["reimbursed"=>["discount-bonus", "subsid-bonus"], "subsid"=>$this->dsub, "rabais-subsid"=>$this->dsub];
    ...
    function display()
    {
        $this->csv = $this->csvHeader(array_merge($this::D1, $this::D2, $this::D3, $this->dsub), "total-subsid");
        foreach($this->master["rabais-subsid"] as $line) {
            if(floatval($line["total-subsid"]) > 0) {
                $this->csv .= "\n".$this->csvLine(array_merge($this::D1, $this::D2, $this::D3, $this->dsub), $line, "total-subsid");
            }
        }
        echo $this->templateDisplay("Total rabais et subsides sur la période", "total-subsid", "total-subsides");
    }
